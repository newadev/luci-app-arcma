...
- **零外部依赖**：核心脚本为纯 POSIX sh，仅使用 BusyBox、Linux sysfs 和 `ip-tiny`（OpenWrt 基础包）
...
| `ip link` | `ip-tiny` (OpenWrt base) | 应用 MAC 地址（自动 fallback 到 `ifconfig`） |
...
opkg update
opkg list-installed | grep -E '^luci-base|^rpcd|^ip-tiny' || true
```
